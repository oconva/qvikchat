 ...
 */
export type DefineChatFlowParams = {
	chatAgent: ChatAgent;
	endpoint: string;
	config?: DefineChatFlowConfig;
};
		...
		async ({ query, chatId }) => {
			...
			try {
				...
				if (config.enableRAG) {
					...
					if (!('retriever' in config)) {
						...
						if (!config.retrieverConfig) {
							return {
								error: 'Error: To enable RAG you must provide either retriever or retriever config.',
							};
						}
						// get retriever using the provided configuration
						const retriever = await getDataRetriever(
				...
				if (
					...
					config.cacheStore.cacheResponse(
						queryHash,
						response.res.text()
					);
				}
